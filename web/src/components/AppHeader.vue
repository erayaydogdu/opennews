<template>
  <header class="hdr" role="banner">
    ...
    <div class="hdr-right">
      <button class="theme-toggle" title="切换日间/夜间模式" @click="onToggleTheme">
      ...
      <span class="hdr-stat-label">条新闻</span>
      <span class="hdr-divider">│</span>
      <span class="hdr-stat">{{ stats.topics }}</span>
      <span class="hdr-stat-label">个主题</span>
      <span class="hdr-divider">│</span>
      <span class="hdr-badge hdr-badge--high">高 {{ stats.high }}</span>
      <span class="hdr-badge hdr-badge--mid">中 {{ stats.mid }}</span>
      <span class="hdr-badge hdr-badge--low">低 {{ stats.low }}</span>
    </div>
  </header>
</template>
